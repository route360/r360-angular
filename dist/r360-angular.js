    ...
    $scope.$watch('travelDistanceRange', function() {
      if ($scope.travelDistanceRange) {
        ...
        $scope.travelDistanceItems = $scope.travelDistanceRange.times
          .map(function(time, i) {
            return { time: time, label: labels[i] || time } });

        $scope.selected = $scope.travelDistanceItems.filter(function(item) {
          return item.time === $scope.model })[0];
      }
    })
